...
<section class="container main">
    ...
    {{-- <div style="background-color:darkgrey;"> --}}
            ...
            <div class="seat rownum">{{$i}}</div>
                @foreach($section[0]->testseat as $seat)
                @if($seat->row==$i)
            <div class="seat @if($seat->seatstatus==0){{"half se"}}@elseif($seat->seatstatus==1){{"none"}}@elseif($seat->seatstatus==3){{"none half"}}@else{{"se"}}@endif"
                data-toggle="tooltip" title="{{$event[0]->event->name}}<br />{{$section[0]->hall->venue->name}}<br />
                سالن&nbsp;{{$section[0]->hall->name}}<br />{{$section[0]->name}}<br />ساعت&nbsp;{{$event[0]->sanse->st_time}}" data-html="true" style="direction:rtl;">
